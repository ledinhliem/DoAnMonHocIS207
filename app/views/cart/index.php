...
<main class="max-w-6xl mx-auto px-8 py-12">
    <h1 class="text-4xl font-black font-headline text-primary mb-8">Giỏ hàng</h1>
            ...
            <div class="lg:col-span-2 space-y-4">
                <?php foreach ($items as $item): ?>
                    <div class="bg-white rounded-2xl border border-outline-variant/30 p-5 flex gap-4 items-center">
                        <img src="<?= htmlspecialchars($item['image']) ?>" alt="<?= htmlspecialchars($item['name']) ?>" class="w-24 h-24 rounded-xl object-cover">
                        ...
                        <div class="flex-1">
                            ...
                            <div class="flex items-center gap-2 mt-3">
                                <form method="POST" action="?url=cart/update">
                                    <input type="hidden" name="ma_san_pham" value="<?= $item['id'] ?>">
                                    <input type="hidden" name="so_luong" value="<?= max(0, $item['quantity'] - 1) ?>">
                                    <button type="submit" class="w-10 h-10 rounded-xl border border-outline-variant text-lg font-bold">-</button>
                                </form>

                                <input type="number" value="<?= $item['quantity'] ?>" class="w-16 text-center rounded-xl border border-outline-variant bg-white" readonly>

                                <form method="POST" action="?url=cart/update">
                                    <input type="hidden" name="ma_san_pham" value="<?= $item['id'] ?>">
                                    <input type="hidden" name="so_luong" value="<?= $item['quantity'] + 1 ?>">
                                    <button type="submit" class="w-10 h-10 rounded-xl border border-outline-variant text-lg font-bold">+</button>
                                </form>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-col items-end gap-3">
                            ...
                            <a href="?url=cart/remove&ma_san_pham=<?= $item['id'] ?>" class="px-3 py-2 rounded-xl border border-red-300 text-red-600 text-sm">
                                Xóa
                            </a>
                        </div>
                    </div>
