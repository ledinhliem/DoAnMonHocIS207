...
<main class="max-w-6xl mx-auto px-8 py-12">
    <h1 class="text-4xl font-black font-headline text-primary mb-8">Order History</h1>
            ...
            <?php foreach (array_reverse($orders) as $order): ?>
                <div class="bg-white rounded-2xl border border-outline-variant/30 p-6">
                    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        <div>
                            <div class="text-sm text-on-surface-variant">Order ID</div>
                            <div class="font-bold text-primary"><?= htmlspecialchars($order['order_id']) ?></div>
                        </div>
                        <div>
                            <div class="text-sm text-on-surface-variant">Created at</div>
                        ...
                        <div>
                            <a href="<?= BASE_URL ?>?url=order/tracking" class="px-4 py-2 rounded-xl bg-primary text-white text-sm">Track</a>
                        </div>
                    </div>
                </div>
            <?php endforeach; ?>
